...
class SalaryHistory::ExcelTable < SalaryHistory::BaseTable
  ...
  URL = 'https://www.belstat.gov.by/upload-belstat/upload-belstat-excel/Oficial_statistika/2019/nach_sr_zarplata-91-19_1912.xlsx'.freeze
       # https://www.belstat.gov.by/upload-belstat/upload-belstat-excel/Oficial_statistika/2019/nach_sr_zarplata-91-19_1911.xlsx
  ...
  def fetch_salaries_grouped_by_year
    ...
    rows = sheet.rows.to_a[4..32].map { |row| row.values }

    rows.each_with_object({}) do |row, obj|
      year = row[0][0..3].to_i
      salaries_per_months = row[1..].map(&:to_i)
      obj[year] = salaries_per_months
    end
  end
end
